...
GridPanel.prototype.createGrid = function (options) {
    ...
    options.onBeforeLoad = function (arg) {
        if (!me.listUrl)
            return false;
        ajaxLoadValue("载入", me.listUrl, arg, function (data) {
            if (!data)
                data = [];
            me.execGridMethod("loadData",data);
        });
        return false;
    }
    
    if (me.isTree) 
        $(me.elementId).treegrid(options);
    else {
        var g = $(me.elementId);
        g.datagrid(options);
...
GridPanel.prototype.createEditor = function () {
    var me = this;
    var ed = new EditorDialog();
    ed.checkReadOnly = function (isAddNew) {
        if (isAddNew) {
...
GridPanel.prototype.doReset = function () {
    ...
    me.doRemote("还原", me.cmdPath +  "state/reset", null, null, function (row) {
        return row.DataState === 0xFF || row.DataState === 0x10;
    });
};
/**
 * 执行废弃操作
...
GridPanel.prototype.initToolbar = function () {
    ...
    me.createToolbarButton("btnQuery", "查询", "list", "icon-search", true, false, function () {
        me.doQuery();
    }, null, "regQuery");

    me.createToolbarButton("btnAdd", "新增", "addnew", "icon-add", me.addNew, false, function () {
        me.processIndex = 0;
        me.execGridMethod("uncheckAll");
        me.doAddNew();
    }, function () {
    ...
    me.createToolbarButton("btnEdit", "查看", "details", "icon-edit", me.edit, true, function () {
        me.editRow();
    }, function () {
        me.edit = null;
    });
    me.createToolbarButton("btnDelete", "删除", "delete", "icon-cancel", (me.editMask & 0x8) === 0x8, true, function () {
        me.doRemove(me);
    });

    me.createToolbarButton("btnEnable", "启用", "disable", "icon-enable", me.stateData, true, function () {
        me.doEnable();
    });
    me.createToolbarButton("btnDisable", "禁用", "enable", "icon-disable", me.stateData, true, function () {
        me.doDisable();
    });
    me.createToolbarButton("btnLock", "锁定", "lock", "icon-lock", me.stateData, true, function () {
        me.doLock();
    });
    me.createToolbarButton("btnDiscard", "废弃", "discard", "icon-discard", me.stateData, true, function () {
        me.doDiscard();
    });
    me.createToolbarButton("btnReset", "恢复", "reset", "icon-reset", me.stateData, true, function () {
        me.doReset();
    });

    me.createToolbarButton("btnValidate", "数据校验", "validate", "icon-validate", me.auditData, true, function () {
        me.doValidate();
    });
    me.createToolbarButton("btnAuditSubmit", "提交审核", "submit", "icon_a_submit", me.auditData, true, function () {
        me.doSubmit();
    });
    me.createToolbarButton("btnPullback", "拉回", "pullback", "icon_a_pullback", me.auditData, true, function () {
        me.doPullback();
    });
    me.createToolbarButton("btnAuditBack", "退回", "back", "icon_a_back", me.auditData, true, function () {
        me.doBack();
    });
    me.createToolbarButton("btnAuditPass", "通过", "pass", "icon_a_pass", me.auditData, true, function () {
        me.doPass();
    });
    me.createToolbarButton("btnAuditDeny", "否决", "deny", "icon_a_deny", me.auditData, true, function () {
        me.doDeny();
    });
    me.createToolbarButton("btnReAudit", "重做", "reaudit", "icon_a_again", me.auditData, true, function () {
        me.doReAudit();
    });
}
